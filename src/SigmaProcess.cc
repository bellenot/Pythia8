...
double SigmaProcess::gg2qqbar() {
  ...
  return CONVERT2MB * (M_PI/sH2) * alphaS2 * nQuark * (sigTS + sigUS);  
...
double SigmaProcess::qqbar2gamgam() {
  ...
  // Calculate. Factor 1/3 from colour matching.
  double charge = ParticleDataTable::charge(id1);
  double sigTU = 2. * (tH2+uH2)/(tH*uH) / 3.;
  flow = 10010000;

  // Answer contains factor 1/2 from identical photons.
  return CONVERT2MB * (M_PI/sH2) * pow2(alphaEM) * pow4(charge) 
    * 0.5 * sigTU;  
