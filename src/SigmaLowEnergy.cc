// SigmaLowEnergy.cc is a part of the PYTHIA event generator.
// Copyright (C) 2023 Torbjorn Sjostrand.
// PYTHIA is licenced under the GNU GPL v2 or later, see COPYING for details.
// Please respect the MCnet Guidelines, see GUIDELINES for details.

// Function definitions (not found in the header) for the SigmaLowEnergy class.
...
void SigmaLowEnergy::init(NucleonExcitations* nucleonExcitationsPtrIn) {
...
double SigmaLowEnergy::sigmaTotal(int idAIn, int idBIn, double eCMIn,
...
double SigmaLowEnergy::sigmaPartial(int idAIn, int idBIn, double eCMIn,
...
bool SigmaLowEnergy::sigmaPartial(int idAIn, int idBIn, double eCMIn,
...
int SigmaLowEnergy::pickProcess(int idAIn, int idBIn, double eCMIn,
  double mAIn, double mBIn) {
...
int SigmaLowEnergy::pickResonance(int idAIn, int idBIn, double eCMIn) {
...
double SigmaLowEnergy::calcRes(int idR) const {
...
//
void SigmaLowEnergy::setConfig(int idAIn, int idBIn, double eCMIn,
  double mAIn, double mBIn) {
...
double SigmaLowEnergy::HPR1R2(double p, double r1, double r2,
...
double SigmaLowEnergy::HERAFit(double a, double b, double n, double c,
...
double SigmaLowEnergy::nqEffAQM(int id) const {
...
double SigmaLowEnergy::factorAQM() const {
  return nqEffAQM(idA) * nqEffAQM(idB) / 9.;
}

double SigmaLowEnergy::totalAQM() const {
  return 40. * factorAQM();
}

double SigmaLowEnergy::elasticAQM() const {
...
double SigmaLowEnergy::meltpoint(int idX, int idM) const {
...
void SigmaCombined::init(SigmaLowEnergy* sigmaLowPtrIn) {

  // Save pointer to low-energy cross sections.
  sigmaLowPtr = sigmaLowPtrIn;

  // Initialize high-energy cross sections.
  sigmaSDL.init( infoPtr);

  // Get parameters for transition from low- to high-energy description.
  eMinHigh   = parm("SigmaCombined:eMinHigh");
  deltaEHigh = parm("SigmaCombined:deltaEHigh");
  eMaxHigh   = eMinHigh + deltaEHigh;
  mp         = particleDataPtr->m0(2212);
...
// Routine to return a collision total cross section for a subcollision.
// mixLoHi = -1 : low-energy; = 0: mixed by energy; = 1 : high-energy.

double SigmaCombined::sigmaTotal(int id1, int id2, double eCM12,
  double m1, double m2, int mixLoHi) {

  // Reject non-hadrons, including nuclei.
  if (!particleDataPtr->isHadron(id1) || !particleDataPtr->isHadron(id2))
    return 0.;

  // Determine bounds of crossover region.
  double eMinNow = eMinHigh + max( 0., m1 - mp) + max( 0., m2 - mp);
  double eMaxNow = eMinNow + deltaEHigh;

  // Pure low energy description.
  if (mixLoHi == -1 || eCM12 < eMinNow)
    return sigmaLowPtr->sigmaTotal( id1, id2, eCM12, m1, m2);

  // Pure high energy description.
  else if (mixLoHi == 1 || eCM12 > eMaxNow)
    return sigmaSDL.sigmaTotal( id1, id2, eCM12 * eCM12, m1, m2);

  // Mixed description.
  else {
    double fracHE = (eCM12 - eMinNow) / deltaEHigh;
    double sigmaLE = sigmaLowPtr->sigmaTotal( id1, id2, eCM12, m1, m2);
    double sigmaHE = sigmaSDL.sigmaTotal( id1, id2, eCM12 * eCM12, m1, m2);
    return (1. - fracHE) * sigmaLE + fracHE * sigmaHE;
  }
...
// Routine to return a collision cross section for a subcollision.
// type = 0 : total; = 1 : nondiff; = 2 : el; = 3 : SD (XB); = 4 : SD (AX);
//      = 5 : DD; = 6 : CD (AXB); = 7 : excitation; = 8 : annihilation;
//      = 9 : resonant.
// mixLoHi = -1 : low-energy; = 0: mixed by energy; = 1 : high-energy.

double SigmaCombined::sigmaPartial(int id1, int id2, double eCM12,
  double m1, double m2, int type, int mixLoHi) {
  ...
  if (abs(type) > 9) {
    if (mixLoHi == 1)
      ...
      return sigmaLowPtr->sigmaPartial(id1, id2, eCM12, m1, m2, type);
  }
  ...
  if (id1 != id1SDL || id2 != id2SDL || eCM12 != eCM12SDL
    || mixLoHi != mixSDL) {

    // Variables.
    int mixNow    = mixLoHi;
    double fracHE = 0.5;
    for (int i = 0; i < 10; ++i) sigSDL[i] = 0.;

    // Decide if in crossover region, and if so relative mix.
    if (mixLoHi == 0) {
      if (eCM12 < m1 + m2 + eMinHigh) mixNow = -1;
      else if (eCM12 > m1 + m2 + eMaxHigh) mixNow = 1;
      else fracHE = (eCM12 - m1 - m2 - eMinHigh) / deltaEHigh;
    }

    // Calculate cross sections at low energies.
    if (mixNow < 1) {
      double fracNow = (mixNow < 0) ? 1. : 1. - fracHE;
      for (int typeNow = 0; typeNow < 10; ++typeNow)
        sigSDL[typeNow] = fracNow * sigmaLowPtr->sigmaPartial( id1, id2,
        eCM12, m1, m2, typeNow);
    }

    // Calculate cross sections at high energies.
    if (mixNow > -1) {
      double fracNow = (mixNow > 0) ? 1. : fracHE;

      // Find hadron masses; for mesons replaced by vector meson ditto.
      int idAbs1 = abs(id1);
      int idAbs2 = abs(id2);
      int idMod1 = (idAbs1 < 100 || idAbs1 > 1000) ? idAbs1
        : 10 * (idAbs1/10) + 3;
      int idMod2 = (idAbs2 < 100 || idAbs2 > 1000) ? idAbs2
        : 10 * (idAbs2/10) + 3;
      if (idAbs1 == 22) idMod1 = 113;
      if (idAbs2 == 22) idMod2 = 113;
      if (idAbs1 == 990) idMod1 = idAbs1;
      if (idAbs2 == 990) idMod2 = idAbs2;
      if (idMod1 != idAbs1) m1 = particleDataPtr->m0(idMod1);
      if (idMod2 != idAbs2) m2 = particleDataPtr->m0(idMod2);

      // Evaluate cross sections.
      sigmaSDL.calcTotEl( id1, id2, eCM12 * eCM12, m1, m2);
      sigmaSDL.calcDiff(  id1, id2, eCM12 * eCM12, m1, m2);
      sigSDL[0] += fracNow * sigmaSDL.sigTot;
      sigSDL[1] += fracNow * sigmaSDL.sigNDtmp;
      sigSDL[2] += fracNow * sigmaSDL.sigEl;
      sigSDL[3] += fracNow * sigmaSDL.sigXB;
      sigSDL[4] += fracNow * sigmaSDL.sigAX;
      sigSDL[5] += fracNow * sigmaSDL.sigXX;
      sigSDL[6] += fracNow * sigmaSDL.sigAXB;
    }
