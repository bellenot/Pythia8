...
//--------------------------------------------------------------------------
  ...
  void HelicityMatrixElement::initPointers(Couplings* sm, ParticleData* pd) {
    smData = sm; particleData = pd;
    for(int i = 0; i <= 5; i++)
...
//--------------------------------------------------------------------------
  ...
  complex HelicityMatrixElement::calculateProductD(vector<HelicityParticle>& p,
						   vector<int>& h1,
						   vector<int>& h2) {
    complex answer(1,0);
...
//--------------------------------------------------------------------------
  ...
  void HelicityMatrixElement::setFermionLine(int position, HelicityParticle& p0,
					     HelicityParticle& p1) {
    vector< Wave4 > u0, u1;
...
//--------------------------------------------------------------------------
  ...
  void HMETwoFermions2Z2TwoFermions::initConstants() {
    ...
    sin2W = smData->sin2thetaW();
    cos2W = smData->cos2thetaW(); 
    // Set the on-shell Z mass and width.
    zG = particleData->mWidth(23);
    zM = particleData->m0(23);
    // Set the vector and axial couplings to the fermions.
    p0CA = smData->af(abs(pID[0]));
    p2CA = smData->af(abs(pID[2]));
    p0CV = smData->vf(abs(pID[0]));
    p2CV = smData->vf(abs(pID[2]));
  }
...
//--------------------------------------------------------------------------
  ...
  void HMETwoFermions2GammaZ2TwoFermions::initPointers(Couplings* sm,
						       ParticleData* pd) {
    zHME.initPointers(sm, pd);
    gHME.initPointers(sm, pd);
  }
...
//--------------------------------------------------------------------------
  ...
  void HMETau2TwoMesonsViaVector::initConstants() {
    ...
    if (abs(pID[2]) == 221) {
      DECAYWEIGHTMAX = 10;
      pM[2] = particleData->m0(211); pM[3] = particleData->m0(311);
    ...
    // Decay through rho resonances (pi^0 + pi^- and K^0 + K^- decays).
      ...
      pM[2] = particleData->m0(111); pM[3] = particleData->m0(211);
...
//--------------------------------------------------------------------------
  ...
  void HMETau2FourPions::initConstants() {
    ...
    pinM  = particleData->m0(111);
    picM  = particleData->m0(211);
