...
void ColConfig::insert( vector<int>& iPartonIn, Event& event) {
  ...
  singlets.push_back( ColSinglet(iBegIn, iEndIn, pSumIn, massIn,
  ...
  if (iInsert < int(singlets.size()) - 1) singlets[iInsert] = 
    ColSinglet(iBegIn, iEndIn, pSumIn, massIn, massExcessIn, 
...
void ColConfig::collect(int iSub, Event& event) {
  ...
  bool inOrder = true;
  for (int i = singlets[iSub].iBegin; i < singlets[iSub].iEnd - 1; ++i) {
    int iFirst = iPartonSave[i];
    if (iFirst < 0) continue;
    int iSecond = iPartonSave[i + 1];
    if (iSecond < 0) iPartonSave[i + 2];
    if (iSecond != iFirst + 1) { inOrder = false; break;}
  }
  if (inOrder) return;
 
  // Copy down system. Update current partons.
  for (int i = singlets[iSub].iBegin; i < singlets[iSub].iEnd; ++i) {
    int iOld = iPartonSave[i];
    if (iOld < 0) continue;
