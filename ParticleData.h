...
public:
  ...
  DecayChannel(double bratIn = 0., int modeIn = 0, int prod0 = 0, 
    ...
    mode(modeIn), nProd(0) {prod[0] = prod0; prod[1] = prod1; 
    prod[2] = prod2; prod[3] = prod3; prod[4] = prod4; prod[5] = prod5; 
    prod[6] = prod6; prod[7] = prod7; 
    for (int i = 0; i < 8; ++i) if (prod[i] != 0 && i == nProd) ++nProd; }  
  DecayChannel(const DecayChannel& chan) : brat(chan.brat), mode(chan.mode),
    nProd(chan.nProd) { for (int i = 0; i < 8; ++i) prod[i] = chan.prod[i]; } 
  DecayChannel& operator=(const DecayChannel& chan) { if (this != &chan)
    { brat = chan.brat; mode = chan.mode; nProd = chan.nProd; 
    for (int i = 0; i < 8; ++i) prod[i] = chan.prod[i];} return *this; }  

  // Member functions for input.
  void branchingRatio(double bratIn) {brat = bratIn;}
  void rescaleBR(double fac) {brat *= fac;} 
  void modeME(int modeIn) {mode = modeIn;} 
  void multiplicity(int multIn)  {nProd = multIn;} 
  void product(int i, int prodIn) {prod[i] = prodIn;}
...
public:
  ...
  ParticleDataEntry(int idIn, string nameIn, string antiNameIn, 
    ...
    { constituentMassCalc(); if (tolower(antiNameIn) == "void") 
    { hasAntiSave = false; } }
  ...
  void setWidth(double widthIn) {widthSave = widthIn;}
  void setRange(double rangeIn) {rangeSave = rangeIn;}
  void setTau0(double tau0In) {tau0Save = tau0In;}
...
public:
  ...
  static bool init(string startFile = "ParticleData.man") ;
  ...
  static void list(ostream& os = cout) ; 
