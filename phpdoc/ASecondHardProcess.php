...
<?php $filepath = $_GET["filepath"];
echo "<a href='ProcessProperties.php?filepath=".$filepath."' target='page'>";?>process properties</a> and 
<?php $filepath = $_GET["filepath"];
echo "<a href='PhaseSpaceCuts.php?filepath=".$filepath."' target='page'>";?>phase space cuts</a> as the first.
Specifically, a <i>pTmin</i> cut for <i>2 -> 2</i> would apply to 
the first and the second hard process alike, and ballpark half of the 
time the second could be generated with a larger <i>pT</i> than the 
first. (Exact numbers depending on the relative shape of the two cross 
sections.) That is, first and second is only used as an administrative
distinction between the two, not as a physics ordering one.
...
and <i>g g -> g g</i> is can trivially be obtained two ways.
