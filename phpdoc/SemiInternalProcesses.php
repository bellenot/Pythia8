...
<code>pT2</code> values (<code>s3 = m3*m3</code> etc.). Overload the 
...
<br>As alternative to the kinematics variables defined above, to be used
for cross section calculations, also the two arrays <code>mME[5]</code> 
and <code>pME[5]</code> can be used, for masses and four-momenta,
respectively. Here indices 0 and 1 are the two incoming beams, and 2
and onwards the outgoing particles. (Note that this differs from the normal
internal labelling, where slot 0 is left empty.) The four-momenta are 
defined in the rest frame of the subcollision, and do not represent the 
final kinematics. Furthermore, for <i>2 -> 2</i> processes of massive 
identical particles (or antiparticles) in the final state, such as 
<i>t tbar</i> or <i>W^+ W^-</i>, the kinematics is here adjusted 
so that the two particles have the same mass, formed as a suitable 
average of the actual Breit-Wigner-distributed masses. This allows the 
evaluation of matrix-element expressions that only have meaning if the 
two have the same mass.  
...
it is assumed that <i>2 -> 2</i> cross sections are encoded as 
...
<i>d(sigmaHat)/d(tHat)</i>.
This switch has no effect on <i>2 -> 1</i> and <i>2 -> 3</i> processes.
In the former one always needs to encode <i>sigmaHat(sHat)</i>,
in the latter always <i>|M|^2</i>. 
...
<i>2 -> 2</i> and <i>2 -> 3</i> processes. However, in case 
...
event record, to make it a <i>2 -> 1 -> 2</i> or <i>2 -> 1 -> 3</i>
...
the non-QCD <i>2 -> 3</i> phase space selection machinery is rather 
