...
<?php $filepath = $_GET["filepath"];
echo "<a href='EventInformation.php?filepath=".$filepath."' target='page'>";?><code>Info::weight()</code></a> 
...
it has been removed in the current version. Nevertheless, should you
wish to use it, the relevant pieces could be picked up from 8.125
and reinserted into a later version. These pieces are:
<ul>
<li>The <code>include/Pythia6Interface.h</code> file, which provides the
interfaces to the PYTHIA 6.4 methods you will need to generate
hard processes.</li>
<li>The <code>htmldoc/AccessPYTHIA6Processes.html</code> file, 
which describes the facility.</li>  
<li>In the <code>examples</code> subdirectory the files 
<code>main51.cc</code>, <code>main52.cc</code>,
<code>main52.ccmnd</code>, <code>main52.fcmnd</code>,
<code>main53.f</code> (!), <code>main54.cc</code> and
<code>main54.cmnd</code> may serve as useful templates.</li>
<li>To run them, you need to use the <code>examples/configure</code> 
and <code>examples/Makefile</code> files, either to replace the current
one, or to borrow the relevant pieces from (i.e. those pertaining
to PYTHIA6 variables in <code>configure</code> and to the 
<code>main51 - main54</code> programs in <code>Makefile</code>).</li>
</ul>
