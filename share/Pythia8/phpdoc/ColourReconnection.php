...
loosely, only an overall saturation is taken into account. The idea is to later 
account for other dynamical effects through colour reconnections. 
...
rely on the two different colour assignments in the beam remnant. There are two,
somewhat motivated, models implemented: the original PYTHIA scheme and a new 
scheme that tries to incorporate more of the colour knowledge from QCD.
...
Therefore the number of reconnection colours for junction formation
is <code>iColours % 3</code>, where iColours refer to the integer of the string.
...
this variable needs to be turned on. Since it is not possible to have different
...
the total <ei>lambda</ei>-measure. The energies are defined in respectively the
dipole 
or junction restframe.
<br/>
